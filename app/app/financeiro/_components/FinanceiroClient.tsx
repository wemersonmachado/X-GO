...
import { FinanceiroAvancado } from "./FinanceiroAvancado";
import { FinanceiroOperacoes } from "./FinanceiroOperacoes";
...
export function FinanceiroClient({ podeAprovar }: { podeAprovar: boolean }) {
  ...
  const [categoryId, setCategoryId] = useState(""), [costCenterId, setCostCenterId] = useState(""), [accountId, setAccountId] = useState("");
  const [catalogs, setCatalogs] = useState<{ categories: Array<{ id: string; name: string; active: boolean; direction: string }>;
    cost_centers: Array<{ id: string; name: string; active: boolean }>;
    accounts: Array<{ id: string; name: string; active: boolean }> }>({ categories: [], cost_centers: [], accounts: [] });
  ...
  useEffect(() => {
    const loadCatalogs = async () => {
      try { setCatalogs(await ler(await fetch("/api/v1/financeiro/catalogos", { cache: "no-store" }))); }
      catch { /* O registro básico continua disponível sem cadastros opcionais. */ }
    };
    const timer = window.setTimeout(() => void loadCatalogs(), 0);
    window.addEventListener("finance-catalogs-changed", loadCatalogs);
    return () => { window.clearTimeout(timer); window.removeEventListener("finance-catalogs-changed", loadCatalogs); };
  }, []);
  ...
  async function criar(event: FormEvent) {
    ...
    try {
      await ler(await fetch(BASE, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ direction, description, amount_cents: cents, currency: "BRL", due_date: dueDate, notes: notes || null,
        category_id: categoryId || null, cost_center_id: costCenterId || null, account_id: accountId || null, competence_date: competenceDate || null }) }));
  ...
  return <main className="mx-auto flex w-full max-w-6xl flex-col gap-6 p-4 sm:p-6">
    ...
    <section className="rounded-xl border bg-card p-4 sm:p-5" aria-label={t("Novo lançamento")}><h2 className="font-semibold">{t("Novo lançamento")}</h2>
      <form onSubmit={criar} className="mt-4 grid gap-3 md:grid-cols-2 lg:grid-cols-5">
        <label className="text-sm">{t("Tipo")}<select value={direction} onChange={(e) => { setDirection(e.target.value as DirecaoFinanceira); setCategoryId(""); }} className="mt-1 h-10 w-full rounded-sm border bg-background px-3"><option value="receivable">{t("Conta a receber")}</option><option value="payable">{t("Conta a pagar")}</option></select></label>
        ...
        <label className="text-sm">{t("Competência")}<Input className="mt-1" type="date" value={competenceDate} onChange={e => setCompetenceDate(e.target.value)} /></label>
        <label className="text-sm">{t("Categoria")}<select className="mt-1 h-10 w-full rounded-sm border bg-background px-3" value={categoryId} onChange={e => setCategoryId(e.target.value)}><option value="">{t("Sem categoria")}</option>{catalogs.categories.filter(item => item.active && item.direction === direction).map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
        <label className="text-sm">{t("Centro de custo")}<select className="mt-1 h-10 w-full rounded-sm border bg-background px-3" value={costCenterId} onChange={e => setCostCenterId(e.target.value)}><option value="">{t("Sem centro de custo")}</option>{catalogs.cost_centers.filter(item => item.active).map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
        <label className="text-sm">{t("Conta financeira")}<select className="mt-1 h-10 w-full rounded-sm border bg-background px-3" value={accountId} onChange={e => setAccountId(e.target.value)}><option value="">{t("Sem conta financeira")}</option>{catalogs.accounts.filter(item => item.active).map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
    ...
    <section className="rounded-xl border bg-card" aria-label={t("Lançamentos")}><div className="flex flex-wrap items-center justify-between gap-3 border-b p-4"><h2 className="font-semibold">{t("Lançamentos")}</h2><div className="flex gap-2"><select aria-label={t("Filtrar lançamentos")} value={status} onChange={(e) => setStatus(e.target.value as typeof status)} className="h-9 rounded-md border bg-background px-3 text-sm"><option value="open">{t("Em aberto")}</option><option value="settled">{t("Baixados")}</option><option value="cancelled">{t("Cancelados")}</option><option value="all">{t("Todos")}</option></select><Button type="button" variant="outline" size="sm" aria-label={t("Atualizar")} onClick={() => void carregar()} disabled={carregando}><ArrowsClockwise size={15} className={carregando ? "animate-spin" : ""} /></Button></div></div>
    ...
    <aside className="rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950"><strong>{t("Confirmação humana obrigatória.")}</strong> {t("O agente prepara propostas financeiras. Pagamentos, reembolsos e transferências exigem aprovação humana e conector autorizado para execução.")}</aside>
    <FinanceiroAvancado podeAprovar={podeAprovar} />
    <FinanceiroOperacoes entries={items} accounts={catalogs.accounts} podeAprovar={podeAprovar} onReload={carregar} />
  </main>;
}
